  import { Link } from "react-router-dom";
import { ArrowUpRight } from "lucide-react";

import { Reveal } from "@/components/animations/Reveal";
import { doctors } from "@/data/doctors";

export function DoctorsPreview() {
  return (
    <section className="py-24 md:py-32">
      <div className="container-page">
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <Reveal>
            <div className="eyebrow">The Team</div>

            <h2 className="mt-3 max-w-2xl font-display text-4xl md:text-5xl">
              Meet the Specialists Behind Every Smile.
            </h2>

            <p className="mt-5 max-w-2xl text-muted-foreground leading-7">
              Our experienced dental specialists combine advanced technology, clinical expertise and
              compassionate care to deliver exceptional treatment for every patient.
            </p>
          </Reveal>

          <Reveal delay={0.1}>
            <Link
              to="/doctors"
              className="inline-flex items-center gap-2 rounded-full border border-border px-6 py-3 text-sm font-medium transition-all hover:border-primary hover:text-primary"
            >
              Meet Our Team
              <ArrowUpRight className="h-4 w-4" />
            </Link>
          </Reveal>
        </div>

        <div className="mt-16 grid gap-8 md:grid-cols-2 xl:grid-cols-3">
          {doctors.slice(0, 3).map((doctor, index) => (
            <Reveal key={doctor.slug} delay={index * 0.08}>
              <Link to="/doctors" className="group block">
                <div className="overflow-hidden rounded-3xl border border-border bg-card">
                  <div className="aspect-[4/5] overflow-hidden">
                    <img
                      src={doctor.image}
                      alt={doctor.name}
                      loading="lazy"
                      className="h-full w-full object-cover transition duration-700 group-hover:scale-105"
                    />
                  </div>

                  <div className="p-6">
                    <div className="eyebrow">{doctor.specialization}</div>

                    <h3 className="mt-2 font-display text-3xl transition-colors group-hover:text-primary">
                      {doctor.name}
                    </h3>

                    <p className="mt-2 text-sm text-muted-foreground">{doctor.designation}</p>

                    <p className="mt-4 line-clamp-2 text-sm leading-6 text-muted-foreground">
                      {doctor.shortBio}
                    </p>

                    <div className="mt-6 flex items-center justify-between">
                      <span className="text-sm font-medium">
                        {doctor.experienceYears}+ Years Experience
                      </span>

                      <span className="text-sm font-semibold text-primary">View Team →</span>
                    </div>
                  </div>
                </div>
              </Link>
            </Reveal>
          ))}
        </div>
      </div>
    </section>
  );
}
